...
import static lusii.lusiiclaimchunks.LusiiClaimChunks.claimedChunksMap;
...
//
	public boolean execute(CommandHandler handler, CommandSender sender, String[] args) {
		if (sender.getPlayer().score < LusiiClaimChunks.getCost(sender.getPlayer().username)) {
			sender.sendMessage("§eInsufficient funds! Need at least " + LusiiClaimChunks.getCost(sender.getPlayer().username) + " to claim!");
			return true;
		}
		...
		if (claimedChunksMap.getOrDefault(sender.getPlayer().username, 0) >= LusiiClaimChunks.maxClaims && LusiiClaimChunks.maxClaims > 0) {
			sender.sendMessage("§eYou've reached the max amount of claims!");
			return true;
		}
